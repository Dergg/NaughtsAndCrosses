...
def turnEnd():
    ...
    if gameOver == True: #Double check to see if the gameOver variable is true
        outputWinner()
...
def playerTurn(i):
    ...
    while choiceValid == False:
        while rowValid == False: #P1 picks their row
            ...
            row = row.upper() #Makes it uppercase to avoid any case sensitivity
            if row == "T":
                rowValid = True
                rowNum = 0
            elif row == "M":
        ...
        while columnValid == False: #P1 picks their column
            ...
            if column == "L":
                ...
                columnValid = True
                rowNum = -1 #This will get caught by the selection statement to not let it get placed.
            else:
                print("Please enter a valid column (Type L, M or R).")
        if grid[rowNum][colNum] == "-" and playerNum == 1 and rowNum != -1: #If this is Player 1 (code 1)
            grid[rowNum][colNum] = "O"
            choiceValid = True
        elif grid[rowNum][colNum] == "-" and playerNum == 0 and rowNum != -1: #If this is Player 2 (code 0)
            grid[rowNum][colNum] = "X"
            choiceValid = True
        elif rowNum == -1:
            print("Turn cancelled. Returning to row selection.") #Cancels the turn and lets you pick again.
            rowValid = False #Rests validity variables
            columnValid = False
            ...
            rowValid = False # Reset valildity variables
            columnValid = False
    turnEnd()
       
def playGame():
    initSetup() #Sets up the game
    global i #Defines turn index
    global gameOver #Defines whether the game is over
    while gameOver == False: #Loops player turn until the game is over
        playerTurn(i) 
        i = i + 1 #Iterate
